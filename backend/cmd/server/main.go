...
func main() {
	...
	auxSchedulerDone := make(chan struct{})
	go func() {
		defer close(auxSchedulerDone)
		service.RunAuxSchedulerLoop(runCtx, time.Duration(cfg.AuxScheduler.IntervalSeconds)*time.Second)
	}()
